...
- **Поддерживаемые платы**: m1p1, m1p2, m2p1, m2p2, z0p2 (RP2350B PiZero2), s2p2 (OldSkoolFRANK).
...
| s2p2 (RP2350B) | PSRAM модуля PGA2350, CS GPIO 47 |

---

## 🛠️ Плата S2 (OldSkoolFRANK)

Плата [OldSkoolFRANK](https://github.com/rh1tech/frank-lab) на модуле **Pimoroni PGA2350** (RP2350B).
По распиновке полностью совпадает с M2 (`m2p2`), поэтому сборка `S2` — это `MURM2`
со следующими отличиями:

- **ЦАП TDA1545A вместо TDA1387.** TDA1545A придерживается старого («японского», EIAJ)
  соглашения: `WS = HIGH` у него левый канал, тогда как в I2S это правый. Битовый поток тот же —
  каналы наоборот. Для него в `audio_i2s.pio` лежит отдельная программа
  `audio_i2s_tda1545a` (`I2S_DAC_TDA1545A`): она держит WS все 16 бит слова и переключает его
  ровно на MSB, без задержки на такт BCK, которую предписывает I2S. Раскладку каналов и
  `i2s_out()` при этом трогать не нужно — обратная полярность WS компенсирует обратный
  порядок половинок. Разводка
  от ЦАП до гнезда на плате обычная и не при чём.
- **Собирается как m2p2** (`PICO_BOARD=pico2`), отдельный заголовок платы не нужен. PSRAM модуля
  на GPIO 47 при этом работает: SpeccyP определяет RP2350B в рантайме (`SYSINFO_PACKAGE_SEL`) и
  сам берёт CS 47, а `gpio_set_function()` адресует регистр пина напрямую, так что
  `NUM_BANK0_GPIOS=30` из заголовка `pico2` ему не мешает. Собирать с `pimoroni_pga2350`
  (`PICO_RP2350A=0`) не нужно и нельзя — плата уходит в бесконечную перезагрузку, проверено
  на железе.
- **Выход VGA/HDMI выбирается перемычками JP3/JP4/JP5 на плате**, прошивка их не видит:
  замкнуто — VGA на J11, разомкнуто — HDMI на J10.

```bash
cmake -B build -DM_BOARD=S2            # SpeccyP_<версия>_s2p2.uf2
cmake -B build -DM_BOARD=S2 -DHDMI_HSTX=ON   # вариант с HDMI на HSTX и HDMI-audio
```
...
## 🧩Дополнительная информация и новое
