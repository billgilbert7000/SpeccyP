...
uint8_t init_psram() {
  //  psram_spi = psram_spi_init_clkdiv(pio1, -1, 1.4, true);//psram_spi_init_clkdiv(pio0, -1, 2.0, true)
   psram_spi = psram_spi_init_clkdiv(PIO_PSRAM, SM_PSRAM, PSRAM_DIV, true);//psram_spi_init_clkdiv(pio0, -1, 2.0, true)
   psram_write32(&psram_spi, 0x313373, 0xDEADBEEF);
   PSRAM_AVAILABLE = 0xDEADBEEF == psram_read32(&psram_spi, 0x313373);
   psram_write32(&psram_spi, 16777216-1, 16); // 16Mb
   psram_write32(&psram_spi, 8388608-1, 8); // 8 Mb
   psram_write32(&psram_spi, 4194304-1, 4); // 4 Mb
   return psram_read32(&psram_spi, 16777216-1);
...
void psram_cleanup() {
   ...
   // for (uint32_t addr32 = (1ul << 20); addr32 < (2ul << 20); addr32 += 4) {
    for (uint32_t addr32 = (0); addr32 < (16384*255); addr32 += 4) {
