...
uint32_t  init_psram_board() {
  ...
  if ( !_psram_size() )
    ...
    return psram_size_board();
}

void deinit_psram_board() 
{
...
void psram_cleanup() {
 ...
 //   for (uint32_t addr32 = (1ul << 20); addr32 < (2ul << 20); addr32 += 4) 
    for (uint32_t addr32 = (0); addr32 < (2ul << 20); addr32 += 4) 
    {
