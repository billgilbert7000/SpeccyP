SPECCY P v1.7.4 
5.08.2026 
Исправление некоторых багов и повышение стабильности работы.
Добавлена поддержка платы S2 (OldSkoolFRANK) на модуле Pimoroni PGA2350 (RP2350B).
Распиновка совпадает с M2, но стоит ЦАП TDA1545A вместо TDA1387: у него
WS = HIGH это левый канал, а в I2S правый, поэтому каналы менялись местами.
Для него в audio_i2s.pio отдельная программа PIO, разводка платы тут не при чём.
Выход VGA/HDMI выбирается перемычками JP3/JP4/JP5 на самой плате.
SpeccyP_1.7.4_s2p2.uf2
SpeccyP_1.7.4_s2p2_hdmi_audio.uf2
---------------------------------
SPECCY P v1.7.2 
30.07.2026 
Исправление некоторых багов и повышение стабильности работы.
если не RP2350 запускается на частоте 378МГц 
в архиве есть speccy_p.ini настроенный на 252МГц 
положить на SD в .config/speccy_p.ini
Для всего зоопарка плат включая ЗОППу ))
SpeccyP_1.7.2_m1p1.uf2
SpeccyP_1.7.2_m1p2.uf2
SpeccyP_1.7.2_m2p1.uf2
SpeccyP_1.7.2_m2p2.uf2
SpeccyP_1.7.2_m2p2_hdmi_audio.uf2
SpeccyP_1.7.2_z0p1.uf2
SpeccyP_1.7.2_z0p2.uf2
SpeccyP_1.7.2_z0p2_pcm5122.uf2    
...
# только на платах с распиновкой M2 (MURM2 и S2).
------------------------------
SPECCY P v1.6.1 
29.04.2026 

FIX - напряжение питания pico U = 1.4В 
    - Работа в режимк HDMI на разных модулях pico2
    + GPIO_DRIVE_STRENGTH_2MA (HDMI)
        
FIX - возможность выбора напряжения ядра для RP2350 
    - в Advanced setup => Voltage 
    - 1.30В, 1.35В, 1.40В, 1.50В, 1.60В 

FIX- возможность настройки некоторых параметров в файле speccy_p.ini
    - который создается автоматически на SD в .config/speccy_p.ini, если его там нет.
    - если что-то пойдет не так, удалить .config/speccy_p.ini  и .config/speccy_p.ini
-----------------------
*speccy_p.ini*
[system]
...
NEW - возможность настройки некоторых пераметров в файле speccy_p.ini
    ...
    - если что-то пойдет не так, удалить speccy_p.ini и speccy_p.cnf с SD 
-----------------------
*speccy_p.ini*
...
Прошивки для платы S2 (OldSkoolFRANK)
 - s2p2 модуль Pimoroni PGA2350 (RP2350B), PSRAM модуля CS GPIO 47
