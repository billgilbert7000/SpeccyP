...
// ===============================================================
// ZX FLASH TIMER CALLBACK
// ===============================================================
bool zx_flash_callback(repeating_timer_t *rt) {
...
void keyboard_and_other(void) {
    ...
    if ((decode_PS2()) | (decode_key(emu_is_stopped())) | (decode_joy())) {
        ...
        #ifndef GENERAL_SOUND
        if (conf.type_sound < HARD_AY) {
            if (F7) {
                msg_bar = 1;
                wait_msg = 1000;
                if ((conf.type_sound == SOFT_AY) | (conf.type_sound == SOFT_TS)) {
                    if (conf.vol_ay > 0) conf.vol_ay--;
                    vol_ay = conf.vol_ay;
                    init_vol_ay();
                }
                if ((conf.type_sound == I2S_AY) | (conf.type_sound == I2S_TS)) {
                    if (conf.vol_i2s > 0) conf.vol_i2s--;
                    vol_ay = conf.vol_i2s;
                    init_vol_ay();
                }
            }
            if (F8) {
                ...
                wait_msg = 1000;
                if ((conf.type_sound == SOFT_AY) | (conf.type_sound == SOFT_TS)) {
                    if (conf.vol_ay < MAX_VOL_PWM) conf.vol_ay++;
                    vol_ay = conf.vol_ay;
                    init_vol_ay();
                }
                if ((conf.type_sound == I2S_AY) | (conf.type_sound == I2S_TS)) {
                    if (conf.vol_i2s < 100) conf.vol_i2s++;
                    vol_ay = conf.vol_i2s;
                    init_vol_ay();
                }
            }
...
void help_zx(void) {
    ...
    draw_rect(0, 20, 318, 200, CL_BLACK, true);
    draw_rect(0, 20, 318, 200, CL_GRAY, false);
    draw_rect(0 + 2, 20 + 2, 318 - 4, 200 - 4, CL_GRAY, false);
    draw_rect(0 + 3, 20 + 3, 318 - 6, 8, CL_GRAY, true);
    draw_text(0 + 10, 20 + 3, "ZX SpeccyP  Help", CL_BLACK, CL_GRAY);
...
void help_keyboard(void) {
    ...
    draw_rect(0, 10, 318, 200, CL_BLACK, true);
    draw_rect(0, 10, 318, 200, CL_GRAY, false);
    draw_rect(0 + 2, 10 + 2, 318 - 4, 200 - 4, CL_GRAY, false);
    draw_rect(0 + 3, 10 + 3, 318 - 6, 8, CL_GRAY, true);
    draw_text(0 + 10, 10 + 3, "ZX Keyboard  [ESC] Exit", CL_BLACK, CL_GRAY);
