...
//===============================================================================================
static void TFT_ILI9341_begin(void)
{
  DISP_CS_SELECT;
  DISP_RST_RESET;
  sleep_ms(200);
...
- Параметры (для ILI9341):
 ...
 // Write_Data(0x00);
  ...
  WriteCommand(0x20 | conf.tft_invert); // инверсия 0x20 или 0x21
  /*
  ...
  */
  ...
  // ili9341 0xE0 переворот на 270 0x20 переворот на 90
  uint8_t r= conf.tft_rotate ? 0xE0 : 0x20;
  Write_Data(r | (conf.tft_rgb<<3)); // Добавлен бит BGR (0x08)
...
//===============================================================================================
static void TFT_ILI9341_IPS(void)
{
  DISP_CS_SELECT;
  DISP_RST_RESET;
  sleep_ms(200);
  ...
  // POWER CONTROL 1
  WriteCommand(0xC0);
  Write_Data(0x25); // Оптимизировано для IPS

  // POWER CONTROL 2
  WriteCommand(0xC1);
  Write_Data(0x11); // Оптимизировано для IPS

  // VCOM CONTROL 1
  WriteCommand(0xC5);
  Write_Data(0x3C); // Оптимизировано 3C // Увеличить VCOMH (улучшает черный цвет)
  Write_Data(0x2C); // Уменьшить VCOML (увеличивает контраст)

  // VCM CONTROL 2
  WriteCommand(0xC7);
  Write_Data(0xC2); // Увеличено с 0x86 до 0xC0 для лучшего сброса // Более высокое значение для лучшего контраста
  
  // DISPLAY INVERSION CONTROL (ВАЖНО ДЛЯ УБРАНИЯ ПОЛОС!)
  WriteCommand(0xB4);
  ...
  WriteCommand(0xB1);
  Write_Data(0x00);
  //Write_Data(0x1B); // 70 Гц - оптимально
  Write_Data(0b10011); //100Гц
  // DISPLAY FUNCTION CONTROL
  // Улучшенные настройки для уменьшения остаточных изображений
  WriteCommand(0xB6); // Изменено с 0x08 - улучшенный контроль сканирования
  Write_Data(0x0A); // Оптимизировано
  Write_Data(0x82);
  Write_Data(0x27);
  Write_Data(0x00);
  ...
  // Оптимизированная гамма-коррекция для IPS 
// POSITIVE GAMMA CORRECTION (оптимизировано для IPS)
WriteCommand(0xE0);
Write_Data(0x0F);
Write_Data(0x26); // Изменено с 0x31
Write_Data(0x24); // Изменено с 0x2B
Write_Data(0x0C);
Write_Data(0x0F); // Изменено с 0x0E
Write_Data(0x08);
Write_Data(0x4B); // Изменено с 0x4E
Write_Data(0xA8); // Изменено с 0xF1
Write_Data(0x3B); // Изменено с 0x37
Write_Data(0x0A); // Изменено с 0x07
Write_Data(0x14); // Изменено с 0x10
Write_Data(0x06); // Изменено с 0x03
Write_Data(0x10); // Изменено с 0x0E
Write_Data(0x09);
Write_Data(0x00);

// NEGATIVE GAMMA CORRECTION (оптимизировано для IPS)
WriteCommand(0xE1);
Write_Data(0x00);
Write_Data(0x1C); // Изменено с 0x0E
Write_Data(0x20); // Изменено с 0x14
Write_Data(0x04); // Изменено с 0x03
Write_Data(0x10); // Изменено с 0x11
Write_Data(0x08); // Изменено с 0x07
Write_Data(0x34); // Изменено с 0x31
Write_Data(0x47); // Изменено с 0xC1
Write_Data(0x44); // Изменено с 0x48
Write_Data(0x05); // Изменено с 0x08
Write_Data(0x0B); // Изменено с 0x0F
Write_Data(0x09); // Изменено с 0x0C
Write_Data(0x2F); // Изменено с 0x31
Write_Data(0x36); // Изменено с 0x36
Write_Data(0x0F);
  
  WriteCommand(0x20 | (!conf.tft_invert & 0x01));

  // MEMORY ACCESS CONTROL
  WriteCommand(0x36);
  uint8_t r = conf.tft_rotate ? 0xE0 : 0x20;
  Write_Data(r | (conf.tft_rgb<<3));
...
//---------------------------------------------------
static void TFT_ST7789_begin(void)
{
  DISP_CS_SELECT;
  DISP_RST_RESET;
  sleep_ms(200);
  ...
  */
  // MEMORY ACCESS CONTROL
  WriteCommand(0x36);
  uint8_t r= conf.tft_rotate ? 0x60 : 0xA0;
  Write_Data(r | (conf.tft_rgb<<3)); // Добавлен бит BGR (0x08)
...
void init_TFT()
{
  gpio_init(TFT_CLK_PIN);
  gpio_init(TFT_DATA_PIN);
...
//gpio_put(TFT_LED_PIN, 1);
 ...
 //conv_2pix_4_to_16 =((uint32_t)conv_arr & 0xfffffc00) + 0x400;// было
 conv_2pix_4_to_16 = (uint32_t*)(((uintptr_t)conv_arr & 0xfffffc00) + 0x400);// DeepSeek
